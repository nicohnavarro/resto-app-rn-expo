...
export default function AddReviewRestaurant({navigation,route}) {
  ...
  const addReview = async () => {
    ...
    if(!responseGetRestaurant.statusResponse){
      setLoading(false);
      toastRef.current.show("Error al obtener el restaurant. Por favor intenta mas tarde.",3000)
      return
    }
    ...
    if(!responseUpdateRestaurant.statusResponse){
      setLoading(false);
      toastRef.current.show("Error al actualizar el restaurant. Por favor intenta mas tarde.",3000)
      return
    }
  ...
  return (
    <KeyboardAwareScrollView style={styles.viewBody}>
      <View style={styles.viewRating}>
        <AirbnbRating
          count={5}
          reviews={["Malo","Regular","Normal","Muy bien","Excelente"]}
          defaultRating={0}
          size={35}
