...
export const getRestaurantsReviews = async (idRestaurant) => {
  const result = {
    statusResponse: true,
    error: null,
    reviews: [],
...
export const searchRestaurants = async (word) => {
  console.log(word);
  const result = { statusResponse: true, error: null, restaurants: [] };
  try {
    result.restaurants = await fireSQL.query(
      `SELECT * FROM restaurants WHERE name LIKE '${word}%'`
    );
  } catch (error) {
    result.statusResponse = false;
    result.error = error;
...
export const getToken = async () => {
  if (!Constants.isDevice) {
    Alert.alert(
      "Debes usar un dispositivo fisico para poder utilizar las notificaciones"
    );
    return;
  }
  ...
  if (finalStatus !== "granted") {
    Alert.alert("Debes dar permiso para poder utilizar las notificaciones");
    return;
  }
...
export const sendEmailResetPasword = async (email) => {
  const result = { statusResponse: true, error: null };
  try {
